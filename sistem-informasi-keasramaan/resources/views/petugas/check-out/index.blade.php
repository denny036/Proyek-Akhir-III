...
@section('judul-halaman')
    <a href="{{ route('petugas.home') }}"><span class="text-gray-600">Home / </a></span>Daftar Check Out Mahasiswa
@endsection
...
@section('table')
    @if (Session::get('success-check-out'))
        <div class="p-4 mb-4 text-sm text-green-700 bg-green-100 rounded-lg dark:bg-green-200 dark:text-green-800"
            role="alert">
            <span class="font-medium font-poppins">{{ Session::get('success-check-out') }}</span>
        </div>
    @endif

    @if (Session::get('fail-check-out'))
        <div class="p-4 mb-4 text-sm text-red-700 bg-red-100 rounded-lg dark:bg-red-200 dark:text-red-800" role="alert">
            <span class="font-medium font-poppins">{{ Session::get('fail-check-out') }}</span>
        </div>
    @endif
    ...
    <td class="py-3 px-6 text-center">
        <div class="flex item-center justify-center">
            <span class="bg-blue-500 text-slate-50 py-1 px-3 rounded-full text-xs font-poppins">
                <a href="{{ route('petugas.detail-check-out', $data->id) }}">Detail</a>
            </span>
        </div>
    </td>
    </tr>
    ...
    <div class="row">
        <div class="col-md-12">
            {{ $daftarRequestCheckOut->links('pagination::tailwind') }}
        </div>
    </div>
