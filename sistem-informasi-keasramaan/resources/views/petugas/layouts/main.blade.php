...
<body class="antialiased bg-gray-100">
    <div class="flex relative" x-data="{navOpen: false}">
        <!-- NAV -->
        <nav class="absolute md:relative w-64 transform -translate-x-full md:translate-x-0 h-screen overflow-y-scroll bg-templateNav transition-all duration-300" :class="{'-translate-x-full': !navOpen}">
            <div class="flex flex-col justify-between h-full">
                ...
                @include('petugas.layouts.partials.navbar')    
                ...
                <div class="text-gray-200 border-gray-800 rounded flex items-center justify-between p-2">
                    <div class="flex items-center space-x-2">
                        ...
                        <img src="{{ asset('/images/avatar/avatar.png') }}" class="w-7 rounded-full" alt="Profile">
                        <h1 class="text-sm font-poppins">{{ Auth::guard('petugas')->user()->nama }}</h1>
                    </div>
                    <a onclick="event.preventDefault(); document.getElementById('logoutForm').submit()" href="{{ route('petugas.logout') }}" class="hover:bg-gray-800 hover:text-white p-2 rounded">
                        <form id="logoutForm" action="{{ route('petugas.logout') }}" method="POST">
                            @csrf 
